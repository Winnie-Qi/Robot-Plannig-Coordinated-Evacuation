...
import numpy as np
from ..methods.dubins_path import DubinsPath
from ..methods.astar import Astar
from ..utils.utils import get_discretized_thetas, round_theta, same_point
...
class HybridAstar:
    ...
    def __init__(self, index, car, grid, reverse, constraints, unit_theta=pi/12, dt=1e-2, check_dubins=1):
        self.car = car
        self.grid = grid
    ...
    def get_children(self, node, heu, extra):
        ...
        for m, phi in self.comb:
            ...
            check_constraints = self.is_equal_with_time(steps_taken,branch[1:]) # 与限制冲突则为True

            # if not safe:
            #     continue
            if not safe or check_constraints:
                continue
            ...
            child.steps = steps_taken + self.drive_steps
            if extra:
    ...
    def best_final_shot(self, open_, closed_, best, cost, d_route, index, steps, constraints, n=10):
        ...
        for t in range(min(n, len(open_))):
            ...
            d_route_, cost_, valid_ = self.dubins.best_tangent(solutions_,index, steps, constraints)
    ...
    def search_path(self, heu=1, extra=False):
        ...
        while open_:
            ...
            if count % self.check_dubins == 0:
                ...
                d_route, cost, valid = self.dubins.best_tangent(solutions,self.index,best.steps,self.constraints)
                if valid :
                    best, cost, d_route = self.best_final_shot(open_, closed_, best, cost, d_route,self.index,best.steps,self.constraints)
                    route = self.backtracking(best) + d_route
                    path,path_points = self.car.get_path(self.start, route)
                    cost += best.g_
                    print('Shortest path: {}'.format(round(cost, 2)))
                    print('Total iteration:', count)
                    
                    return path, path_points, closed_
        ...
        print("Hybrid A* pathfinding没有valid的")
        return None, None, None


    def is_equal_with_time(self,steps_taken,branch):
        time = [i for i in range(steps_taken,steps_taken+self.drive_steps)]
        if not self.constraints[self.index].vertex_constraints:
            return False
        for i in self.constraints[self.index].vertex_constraints:
            if i.time in time:
                if self.is_equal_except_time(i,branch,steps_taken):
                    return True
        return False

    def is_equal_except_time(self,i,branch,steps_taken):
        location1 = i.location
        location2 = branch[i.time-steps_taken]
        dist = np.linalg.norm(
            np.array([location1[0], location1[1]]) - np.array([location2[0], location2[1]]))
