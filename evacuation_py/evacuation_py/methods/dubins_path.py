...
from ..utils.utils import transform, directional_theta, distance
...
class DubinsPath:
    ...
    def find_tangents(self, start_pos, end_pos):
        ...
        self.end_pos = end_pos
        # self.end_angle = [0,-pi/6,-pi/3,-pi/2,-2*pi/3,-5*pi/6,-pi]
        self.end_angle = [0,-pi/8,-pi/4,-3*pi/8,-pi/2,-5*pi/8,-3*pi/4,-7*pi/8,-pi]
        ...
        solutions = []
        for theta in theta2:
            self.lc2 = transform(x2, y2, 0, self.r, theta, 1)
            self.rc2 = transform(x2, y2, 0, self.r, theta, 2)
            solution = [self._LSL(theta), self._LSR(theta), self._RSL(theta), self._RSR(theta)]
            solution = [s for s in solution if s is not None]
    ...
    def _LSL(self, end_angle):

        lsl = Params(self.direction['LSL'],end_angle)
    ...
    def _LSR(self,end_angle):

        lsr = Params(self.direction['LSR'],end_angle)
    ...
    def _RSL(self,end_angle):

        rsl = Params(self.direction['RSL'],end_angle)
    ...
    def _RSR(self,end_angle):

        rsr = Params(self.direction['RSR'],end_angle)
    ...
    def best_tangent(self, solutions,index,steps,constraints):
            ...
            for goal, phi, m in route:
                while True:
                    ...
                    if same_point(pos[:2], goal[:2]):
                        pos = goal
                        break
                    path_points.append(pos[:2])

            conflict = space_time_checker(path_points,index,steps,constraints)
            if conflict:
                continue
...
def space_time_checker(path_points,index,steps,constraints):
    if not constraints[index].vertex_constraints:
        return False
    time = [i for i in range(steps,steps+len(path_points))]
    for i in constraints[index].vertex_constraints:
        if i.time in time:
            if is_equal_except_time(i,path_points,steps):
                return True
    return False

def is_equal_except_time(i,branch,steps_taken):
    location1 = i.location
    location2 = branch[i.time-steps_taken]
    dist = np.linalg.norm(
        np.array([location1[0], location1[1]]) - np.array([location2[0], location2[1]]))
    return dist <= Distance_threshhold